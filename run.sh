...
for i in `seq ${MEASUREMENT_ROUNDS}`; do
  ab -k -c ${PARALLEL_USER_COUNT} \
        ...
        -u ${MOCK_TICKET_REQUEST} -T application/json \
        -e ticket/measurements/mock_baseline-${i}.csv \
        ${MOCK_BASE_URL}${MOCK_TICKET_PATH} > ticket/measurements/mock_baseline-${i}
done
...
#  ab -k -c 10 \
#     -n 1000 \
#     -u ${SERVICE_REPLY_REQUEST} -m post -T application/json \
